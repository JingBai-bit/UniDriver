...
# Reports accuracy, macro metrics, and confusion matrices.
...
def _multiclass_metrics_from_preds(
    ...
    """
    if y_true.numel() == 0:
        return {"macro_recall": 0.0, "macro_precision": 0.0, "macro_f1": 0.0}
    ...
    macro_recall = recall.mean().item() * 100.0
...
def _format_dual_head_eval_metrics(metrics: Dict[str, float]) -> str:
    return (
        ...
        "MacroR {macro_recall:.3f} MacroP {macro_precision:.3f} MacroF1 {macro_f1:.3f} //// "
        "AccFC@1 {acc1_fc:.3f} AccFC@5 {acc5_fc:.3f} "
        "MacroR_FC {macro_recall_fc:.3f} MacroP_FC {macro_precision_fc:.3f} MacroF1_FC {macro_f1_fc:.3f}"
    ).format(**metrics)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "e.g. acc1, macro_recall, macro_precision, macro_f1, "
            "acc1_fc, macro_recall_fc, macro_precision_fc, macro_f1_fc"
        ),
    )
