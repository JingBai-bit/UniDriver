...
def main():
    ...
    model = models_adapter.__dict__[args.model](num_classes=DATASETS[args.dataset]['NUM_CLASSES'],
                                                num_frames=args.num_frames, mlm_head_len=mlm_head_len).cuda().train()
    ...
    def evaluate(log_stats=None):
        ...
        for data, labels in metric_logger.log_every(dataloader_val, 10, header):
            ...
            with torch.cuda.amp.autocast():
                ...
                num_classes = scores.size(1)  # 获取类别数量
                k = min(5, num_classes)  # 取类别数和5中的较小值
                if k > 0:
                    acc5_fc = (scores.topk(k, dim=1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                else:
                    acc5_fc = 0.0
                ...
                num_classes = similarity.size(1)  # 获取类别数量
                k = min(5, num_classes)  # 取类别数和5中的较小值
                if k > 0:
                    acc5 = (similarity.topk(k, dim=1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                else:
                    acc5 = 0.0
        ...
        print(
            '* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f}////AccFC@1 {top1_fc.global_avg:.3f} AccFC@5 {top5_fc.global_avg:.3f}'
            .format(top1=metric_logger.acc1, top5=metric_logger.acc5, top1_fc=metric_logger.acc1_fc,
                    top5_fc=metric_logger.acc5_fc))
        ...
        for step, (data, labels) in enumerate(metric_logger.log_every(dataloader_train, args.print_freq, header)):
            ...
            for i in range(data.shape[0]):
                if random.random() < 0:
                    ...
                    mlm_input_tokens_id, mlm_label = get_masked_sample(classes_name[i][1], mlm_label_dict,
                                                                       masked_rate=0.2)
            ...
            with torch.amp.autocast('cuda'):
                ...
                logits_per_image, logits_per_text, image_embedding, text_mlm_inputs, image_mlm_inputs, logits = model(
                    data, mlm_input_tokens_ids)
                acc1_fc = (logits.topk(1, dim=1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                num_classes = logits.size(1)
                k = min(5, num_classes)
                if k > 0:
                    acc5_fc = (logits.topk(k, dim=1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                else:
                    acc5_fc = 0.0
                ...
                with torch.no_grad():
                    ...
                    # 检查并修复NaN/Inf值
                    if torch.isnan(text_norm).any() or torch.isinf(text_norm).any():
                        print("Warning: NaN/Inf detected in text_norm, replacing with 1.0")
                        text_norm = torch.nan_to_num(text_norm, nan=1.0, posinf=1.0, neginf=1.0)
                    text_norm = text_norm.clamp(min=1e-8)
                    text_features = text_features / text_norm
                similarity = (100.0 * image_embedding @ text_features.T)
                similarity = similarity.softmax(dim=-1)
                acc1 = (similarity.topk(1, dim=-1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                # acc5 = (similarity.topk(5, dim=-1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                num_classes = similarity.size(-1)
                k = min(5, num_classes)
                if k > 0:
                    acc5 = (similarity.topk(k, dim=-1)[1] == labels.view(-1, 1)).sum(dim=-1).float().mean().item() * 100
                else:
